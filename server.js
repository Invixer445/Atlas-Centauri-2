...
// directory. On Railway that filesystem is EPHEMERAL: every deploy builds a new
// container and the file goes with the old one. So the bot has never once resumed from
// saved state. Every push produced "[LOAD] No backup — starting fresh", which in turn
// meant the boot sync had no idea which broker positions were core holdings, adopted
// all of them into the trading book with stop-losses, and then bought a SECOND copy of
...
// How long a live basket must be held before a new proposal may replace it. The
    ...
    if (CORE_HOLD_ON && Date.now() - _lastBasketProposalAt > BASKET_PROPOSAL_INTERVAL_MS) {
      ...
      try {
        ...
        if (prop) {
          ...
          // MINIMUM HOLD. The prompt asks for a basket to hold for MONTHS, but the
          // proposal runs every 24h and used to be adopted every time — so a name could
          // be bought and sold out on nothing more than the model sampling differently
          // two days running. Observed 2026-09-01: the new basket dropped PFE, which was
          // the single best performer in the book at +$2.70, and added five names. That
          // is ~$500 of turnover generated by an LLM call, paying spread both ways, with
          // no signal behind it. Proposals are still made, logged and scored on every
          // cycle — the register needs that — but money only moves once the current
          // basket has been held long enough for the change to mean something.
          const heldMs = Date.now() - (_lastBasketSwapAt || 0);
          const firstEver = !_venusBasketReceived;
          const maySwap = firstEver || heldMs >= CORE_BASKET_MIN_HOLD_MS;
          if (CORE_BASKET_SOURCE === 'venus' && !maySwap) {
            const daysLeft = (CORE_BASKET_MIN_HOLD_MS - heldMs) / 86400000;
            console.log(`[VENUS]    proposal LOGGED but not adopted — the live basket is ` +
              `${(heldMs / 86400000).toFixed(1)}d old and the minimum hold is ` +
              `${(CORE_BASKET_MIN_HOLD_MS / 86400000).toFixed(0)}d (${daysLeft.toFixed(1)}d to go). ` +
              `A basket meant to last months must not be churned by a daily model call.`);
          }
          if (CORE_BASKET_SOURCE === 'venus' && maySwap) {
  ...
  try {
    ...
    if (pos.ok) {
      for (const bp of pos.positions) {
        ...
        // THIS GUARD USED TO ASK THE LEDGER, WHICH IS THE ONE THING A WIPE DESTROYS.
        // It checked portfolio.coreHolding[symbol] — but this sync exists FOR the
        // state-wipe case, and after a wipe coreHolding is empty, so the guard never
        // fired. Measured 2026-09-01: all six core holdings adopted into the trading
        // book with stop-losses, the core then bought a second copy of five of them
        // (quantities reconciled to six decimals), cash drained $429.66 → $0.63, and
        // portfolio heat hit ~83% against a 50% cap.
        //
        // The replacement does not depend on saved state:
        //  · While the phase gate holds trading LOCKED, ATLAS cannot have opened a
        //    trade at all — so every position at the broker is a core holding by
        //    definition. That is a proof, not a heuristic.
        //  · Otherwise a current basket member is core; a non-member is a real trade.
        const ledgerSaysCore = !!(portfolio.coreHolding && portfolio.coreHolding[bp.symbol]);
        const tradingImpossible = PHASE_GATE_ENABLED && tradingPhaseLocked();
        const isBasketMember = CORE_HOLD_SYMBOLS.includes(bp.symbol);
        if (CORE_HOLD_ON && (ledgerSaysCore || tradingImpossible || isBasketMember)) {
          if (!ledgerSaysCore) {
            // Not in the ledger, so it must be RECORDED as core — skipping alone would
            // leave the shares invisible to ATLAS, and the core would buy them again.
            portfolio.coreHolding = portfolio.coreHolding || {};
            portfolio.coreHolding[bp.symbol] = {
...
function buildStateObject() {
  return {
    ...
    // Without this the minimum-hold clock restarts on every deploy, and a bot that is
...
function evaluateAndTrade() {
  ...
  if (riskSystem.riskLevel === 'elevated') {
    if (Date.now() - evaluateAndTrade._lastRiskLog > 60000) {
      ...
      // Name what FAILED, with its number. "Elevated [drawdown, dailyLoss]" listed the
      // checks that PASSED, which reads as exactly the opposite of what it means — the
      // one time it mattered it sent the diagnosis after the wrong two metrics while
      // portfolio heat was the check actually blocking every entry.
      const all = { drawdown: riskSystem.currentDrawdown, heat: riskSystem.portfolioHeat, dailyLoss: dailyLossPct };
      const limits = { drawdown: riskSystem.maxDrawdown, heat: riskSystem.maxPortfolioHeat, dailyLoss: riskSystem.dailyLossLimit };
      const failed = Object.keys(all).filter(k => !riskSystem.checksPassing.includes(k))
        .map(k => `${k} ${(all[k] * 100).toFixed(1)}% > ${(limits[k] * 100).toFixed(0)}%`);
      console.log(`[RISK] Entries blocked — FAILING: ${failed.join(' · ')}`);
    }
    return;
  }
